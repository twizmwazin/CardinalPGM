        ...
        for (Element regionElement : match.getDocument().getRootElement().getChildren("regions")) {
            for (Element applied : regionElement.getChildren("apply")) {
                ...
                if (applied.getAttributeValue("kit") != null) {
                    results.add(new KitRegion(region, (applied.getAttributeValue("filter") != null ? getFilter(applied.getAttributeValue("filter")) : getFilter("always")), message, KitCollection.getKitByName(applied.getAttributeValue("kit"))));
                }
            }
        }
