    ...
    public void onPlayerLeftClick(PlayerAttackEntityEvent event) {
        if (event.getLeftClicked() instanceof TNTPrimed) {
            if (TntTracker.getWhoPlaced(event.getLeftClicked()) != null) {
                ...
                Optional<TeamModule> playerTeam = Teams.getTeamOrPlayerByPlayer(Bukkit.getPlayer(player));
                if (Bukkit.getOfflinePlayer(player).isOnline()) {
                    if (playerTeam.isPresent() && playerTeam.get().equals(Teams.getTeamOrPlayerByPlayer(event.getPlayer()).orNull()) && Teams.getTeamOrPlayerByPlayer(Bukkit.getPlayer(player)) != null && !playerTeam.get().isObserver()) {
                        if (!event.getLeftClicked().getLocation().getBlock().isLiquid()) {
                            if (!Bukkit.getPlayer(player).equals(event.getPlayer())) {
                                ...
                                channel.sendMessage("[" + ChatColor.GOLD + "A" + ChatColor.WHITE + "] " + Teams.getTeamOrPlayerByPlayer(event.getPlayer()).get().getColor() + event.getPlayer().getDisplayName() + ChatColor.RESET + " defused " + playerTeam.get().getColor() + Bukkit.getPlayer(player).getDisplayName() + ChatColor.RESET + "'s " + ChatColor.DARK_RED + "TNT");
                            }
                        } else {
    ...
    public void onPlayerRightClick(PlayerInteractEntityEvent event) {
        if (event.getRightClicked() instanceof TNTPrimed && event.getPlayer().getItemInHand() != null && event.getPlayer().getItemInHand().getType().equals(Material.SHEARS)) {
            if (TntTracker.getWhoPlaced(event.getRightClicked()) != null) {
                ...
                Optional<TeamModule> playerTeam = Teams.getTeamOrPlayerByPlayer(Bukkit.getPlayer(player));
                if (Bukkit.getOfflinePlayer(player).isOnline()) {
                    if (event.getPlayer().hasPermission("tnt.defuse") || Teams.getTeamOrPlayerByPlayer(Bukkit.getPlayer(player)).orNull() == Teams.getTeamOrPlayerByPlayer(event.getPlayer()).orNull()) {
