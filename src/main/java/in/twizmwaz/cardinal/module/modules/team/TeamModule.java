...
public class TeamModule<P extends Player> extends ArrayList<Player> implements Module {
    ...
    public boolean add(Player player, boolean force, boolean message) {
        ...
        if (message) {
            event.getPlayer().sendMessage(ChatColor.WHITE + new LocalizedChatMessage(ChatConstant.GENERIC_JOINED, event.getNewTeam().getCompleteName()).getMessage(event.getPlayer().getLocale()));
        }
