        ...
        if (event.getEntity() instanceof Player) {
            if (event.getCause().equals(DamageCause.PROJECTILE)) {
                if (((Projectile) event.getDamager()).getShooter() instanceof OfflinePlayer) {
                    ...
                    Bukkit.getServer().getPluginManager().callEvent(new TrackerDamageEvent((Player) event.getEntity(), damager, (damager.isOnline() ? ((Player) damager).getItemInHand() : null), Type.SHOT));
                }
                ...
                if (TntTracker.getWhoPlaced(event.getDamager()) != null) {
                    ...
                    Bukkit.getServer().getPluginManager().callEvent(new TrackerDamageEvent((Player) event.getEntity(), damager, (damager.isOnline() ? ((Player) damager).getItemInHand() : null), Type.BLOWN));
                }
            } else {
                if (event.getDamager() instanceof OfflinePlayer) {
                    OfflinePlayer damager = (OfflinePlayer) event.getDamager();
                    Bukkit.getServer().getPluginManager().callEvent(new TrackerDamageEvent((Player) event.getEntity(), damager, (damager.isOnline() ? ((Player) damager).getItemInHand() : null), Type.KNOCKED));
                }
            }
        }
    }
