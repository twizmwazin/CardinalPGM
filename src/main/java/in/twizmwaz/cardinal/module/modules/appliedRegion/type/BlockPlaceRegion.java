...
public class BlockPlaceRegion extends AppliedRegion {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        if (!event.isCancelled() && region.contains(new BlockRegion(null, event.getBlockPlaced().getLocation().toVector().add(new Vector(0.5, 0.5, 0.5)))) && filter.evaluate(event.getPlayer(), event.getBlockPlaced()).equals(FilterState.DENY)) {
    ...
    public void onBucketEmpty(PlayerBucketEmptyEvent event) {
        ...
        if (!event.isCancelled() && region.contains(new BlockRegion(null, event.getBlockClicked().getRelative(event.getBlockFace()).getLocation().toVector().add(new Vector(0.5, 0.5, 0.5)))) && filter.evaluate(event.getPlayer(), newMaterial).equals(FilterState.DENY)) {
            event.setCancelled(true);
            ChatUtils.sendWarningMessage(event.getPlayer(), message);
        }
    }
