    ...
    public static void admin(final CommandContext cmd, CommandSender sender) throws CommandException {
        if (sender instanceof Player) {
            if (cmd.argsLength() == 0) {
                ...
                sender.sendMessage(ChatColor.YELLOW + "Changed default channel to administrator chat");
            }
            if (cmd.argsLength() > 0) {
                String message = assembleMessage(cmd);
                if (message.trim().equals("")) return;
    ...
    public static void team(final CommandContext cmd, CommandSender sender) throws CommandException {
        if (sender instanceof Player) {
            if (cmd.argsLength() == 0) {
                ...
                sender.sendMessage(ChatColor.YELLOW + "Changed default channel to team chat.");
            }
            if (cmd.argsLength() > 0) {
