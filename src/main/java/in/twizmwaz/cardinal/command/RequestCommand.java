...
public class RequestCommand {
    ...
    public static void request(final CommandContext cmd, CommandSender sender) throws CommandException {
        ChatUtils.getAdminChannel().sendMessage("[" + ChatColor.GOLD + "A" + ChatColor.WHITE + "] " + TeamUtils.getTeamColorByPlayer((Player) sender) + ((Player) sender).getDisplayName() + ChatColor.YELLOW + "has requested" + ChatColor.RESET + ": " + cmd.getJoinedStrings(0));
