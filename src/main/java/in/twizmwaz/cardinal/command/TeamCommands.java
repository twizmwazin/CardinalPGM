...
public class TeamCommands {
    ...
    public static void force(final CommandContext cmd, CommandSender sender) throws CommandException {
        if (Bukkit.getPlayer(cmd.getString(0)) != null) {
            Optional<TeamModule> team = TeamUtils.getTeamByName(cmd.getJoinedStrings(1));
            if (team.isPresent()) {
                if (!team.get().contains(Bukkit.getPlayer(cmd.getString(0)))) {
                    ...
                    throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_ALREADY_ON_TEAM,
                            TeamUtils.getTeamByPlayer(Bukkit.getPlayer(cmd.getString(0))).get().getColor() + Bukkit.getPlayer(cmd.getString(0)).getName() + ChatColor.RED,
                            TeamUtils.getTeamByPlayer(Bukkit.getPlayer(cmd.getString(0))).get().getCompleteName()).getMessage(((Player) sender).getLocale()));
            } else {
    ...
    public static void shuffle(final CommandContext cmd, CommandSender sender) throws CommandException {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (TeamUtils.getTeamByPlayer(player).isPresent()) {
                if (!TeamUtils.getTeamByPlayer(player).get().isObserver()) {
                    playersToShuffle.add(player);
                    TeamModule observers = TeamUtils.getTeamById("observers").get();
        ...
        while (playersToShuffle.size() > 0) {
            ...
            Optional<TeamModule> team = TeamUtils.getTeamWithFewestPlayers(GameHandler.getGameHandler().getMatch());
    ...
    public static void size(final CommandContext cmd, CommandSender sender) throws CommandException {
        ...
        if (team.isPresent()) {
            ...
            sender.sendMessage(new LocalizedChatMessage(ChatConstant.GENERIC_TEAM_SIZE_CHANGED, TeamUtils.getTeamByName(cmd.getString(0)).get().getCompleteName() + ChatColor.WHITE, ChatColor.AQUA + cmd.getString(1)).getMessage(ChatUtils.getLocale(sender)));
        } else {
            throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_NO_TEAM_MATCH).getMessage(ChatUtils.getLocale(sender)));
        }
    }
