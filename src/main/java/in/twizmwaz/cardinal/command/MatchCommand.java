...
public class MatchCommand {
    ...
    public static void match(final CommandContext args, CommandSender sender) throws CommandException {
        ...
        if (ScoreModule.matchHasScoring()) {
            ...
            if (ScoreModule.getTimeLimit() != 0) timeRemaining = ScoreModule.getTimeLimit() - MatchTimer.getTimeInSeconds();
            sender.sendMessage(ChatColor.DARK_AQUA + "Score: " + score + "  " + (ScoreModule.getTimeLimit() != 0 ? ChatColor.RED + StringUtils.formatTime(timeRemaining) : ChatColor.GREEN + StringUtils.formatTime(MatchTimer.getTimeInSeconds())));
        }
    }
