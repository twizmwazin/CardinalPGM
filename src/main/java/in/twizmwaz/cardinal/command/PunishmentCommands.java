...
public class PunishmentCommands {
    ...
    public static void kick(CommandContext cmd, CommandSender sender) throws CommandException {
        ...
        if (kicked != null) {
            ...
            Bukkit.broadcastMessage((sender instanceof Player ? Teams.getTeamColorByPlayer((Player) sender) + ((Player) sender).getDisplayName() : ChatColor.YELLOW + "*Console") + ChatColor.GOLD + " \u00BB Kicked \u00BB " + Teams.getTeamColorByPlayer(kicked) + kicked.getDisplayName() + ChatColor.GOLD + " \u00BB " + reason);
    ...
    public static void ban(CommandContext cmd, CommandSender sender) throws CommandException {
        ...
        if (banned.isOnline()) {
            ...
            Bukkit.broadcastMessage((sender instanceof Player ? Teams.getTeamColorByPlayer((Player) sender) + ((Player) sender).getDisplayName() : ChatColor.YELLOW + "*Console") + ChatColor.GOLD + " \u00BB Permanent Ban \u00BB " + Teams.getTeamColorByPlayer(banned) + onlineBanned.getDisplayName() + ChatColor.GOLD + " \u00BB " + reason);
            ...
            Bukkit.broadcastMessage((sender instanceof Player ? Teams.getTeamColorByPlayer((Player) sender) + ((Player) sender).getDisplayName() : ChatColor.YELLOW + "*Console") + ChatColor.GOLD + " \u00BB Permanent Ban \u00BB " + Teams.getTeamColorByPlayer(banned) + banned.getName() + ChatColor.GOLD + " \u00BB " + reason);
            Bukkit.getBanList(BanList.Type.NAME).addBan(cmd.getString(0), ChatColor.RED + "Permanently Banned" + ChatColor.GOLD + "  \u00BB  " + reason, null, sender.getName());
        } else
            throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_PLAYER_ALREADY_BANNED).getMessage(ChatUtil.getLocale(sender)));
        ...
        if (player != null) {
            if (!(!sender.isOp() && player.isOp())) {
                if (!GameHandler.getGameHandler().getMatch().getModules().getModule(PermissionModule.class).isMuted(player)) {
                    ...
                    throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_PLAYER_ALREADY_MUTED).getMessage(ChatUtil.getLocale(sender)));
            } else
                throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_PLAYER_NOT_AFFECTED).getMessage(ChatUtil.getLocale(sender)));
        } else
            throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_NO_PLAYER_MATCH).getMessage(ChatUtil.getLocale(sender)));
        ...
        if (player != null) {
            if (!(!sender.isOp() && player.isOp())) {
                if (GameHandler.getGameHandler().getMatch().getModules().getModule(PermissionModule.class).isMuted(player)) {
                    ...
                    throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_PLAYER_NOT_MUTED).getMessage(ChatUtil.getLocale(sender)));
            } else
                throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_PLAYER_NOT_AFFECTED).getMessage(ChatUtil.getLocale(sender)));
        } else
            throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_PLAYER_NOT_FOUND).getMessage(ChatUtil.getLocale(sender)));
    }
